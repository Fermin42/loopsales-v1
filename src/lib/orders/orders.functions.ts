...
export const saveOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (productIds.length > 0) {
      ...
      // Estados que SÍ consumen stock (no liberan inventario):
      // todo lo que no es draft/pending/cancelled/voided/rejected.
      const FREE_STATUSES = new Set(["draft", "pending", "cancelled", "voided", "rejected"]);
      for (const r of ((resvRows ?? []) as unknown) as Row[]) {
        const ord = Array.isArray(r.order) ? r.order[0] : r.order;
        if (!ord) continue;
        if (FREE_STATUSES.has(ord.status)) continue;
...
export const invoiceOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Recalcular total alineado con el cálculo de Siigo, item por item:
    // por línea: base = round2(qty * price); tax = round2(base * rate / 100).
    // Usamos exactamente los siigoItems que vamos a enviar para evitar diferencias de redondeo.
    let computedSubtotal = 0;
    let computedTax = 0;
    siigoItems.forEach((sit, idx) => {
      const orig = items[idx];
      const qty = Number(sit.quantity as number) || 0;
      const price = Number(sit.price as number) || 0;
      const base = round2(qty * price);
      computedSubtotal += base;
      const rate = orig?.is_gift ? 0 : Number(orig?.tax_rate) || 0;
      if (rate > 0) computedTax += round2(base * rate / 100);
    });
    const totalWithTax = round2(computedSubtotal + computedTax);
    // Fecha del día en zona horaria de Bogotá (Colombia). ISO evita corrimientos por UTC.
    ...
    // La nota del pedido es INTERNA: no se envía a Siigo. Solo se envía la nota configurada por admin.
    const defNote = (defaultNote ?? "").trim();
    const observations = defNote || undefined;
...
export const cancelOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: row } = await supabaseAdmin
      .from("orders").select("status, seller_id").eq("id", data.id).maybeSingle();
    if (!row) throw new Error("Pedido no encontrado");
    const admin = await isAdmin(context.userId);
    const owner = row.seller_id === context.userId;
    // El vendedor dueño puede cancelar borradores/pendientes; admin puede cancelar
    // hasta antes de facturar.
    const allowedForOwner = ["draft", "pending"];
    const allowedForAdmin = ["draft", "pending", "confirmed"];
    if (admin) {
      if (!allowedForAdmin.includes(row.status))
        throw new Error("No se puede cancelar un pedido en este estado (usa una solicitud de anulación)");
    } else if (owner) {
      if (!allowedForOwner.includes(row.status))
        throw new Error("Solo puedes cancelar pedidos en borrador o pendientes (para los demás estados, envía una solicitud de anulación)");
    } else {
