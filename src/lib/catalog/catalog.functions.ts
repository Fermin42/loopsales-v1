...
import { SIIGO_CITY_CATALOG } from "@/lib/catalog/siigo-cities.generated";
...
const LocalCustomerSchema = z.object({
  ...
  country_code: z.string().trim().max(5).default("COL"),
});
...
export const createLocalCustomer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const row = {
      ...
      state_name: null,
      country_code: data.country_code || "COL",
...
// Catálogo local oficial de ciudades Siigo, alimentado desde el Excel de Países/Departamentos/Ciudades.
let _siigoCitiesCache: Array<{ code: string; name: string; state_name: string; state_code: string; country_code: string }> | null = null;
async function loadSiigoCities() {
  if (_siigoCitiesCache && _siigoCitiesCache.length > 0) return _siigoCitiesCache;
  _siigoCitiesCache = SIIGO_CITY_CATALOG.map((c) => ({
    code: c.city_code,
    name: c.city,
    state_name: c.state,
  ...
  return _siigoCitiesCache;
}
function norm(s: string) {
  return s.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().trim();
}
async function resolveSiigoCity(name: string | null, code: string | null) {
  if (code && code.length >= 3) {
    const cities = await loadSiigoCities();
    const hit = cities.find((c) => c.code === code);
    if (hit) return hit;
  }
  if (name && name.trim()) {
    const cities = await loadSiigoCities();
    ...
    const partial = cities.find((c) => norm(c.name).startsWith(n)) ?? cities.find((c) => norm(c.name).includes(n));
    if (partial) return partial;
  }
  return null;
}

// Búsqueda pública (autenticada) para autocompletar ciudades de Siigo en el front.
export const searchSiigoCities = createServerFn({ method: "GET" })
  .middleware([attachAuthHeader, requireSupabaseAuth])
  ...
  .handler(async ({ data }) => {
    ...
    const filtered = !q
      ? cities.slice(0, 50)
      : cities
          .filter((c) => norm(`${c.name} ${c.state_name} ${c.code}`).includes(q))
          .sort((a, b) => {
            const ai = norm(a.name).startsWith(q) ? 0 : 1;
            const bi = norm(b.name).startsWith(q) ? 0 : 1;
            return ai - bi || a.name.localeCompare(b.name);
          })
          .slice(0, 50);
    return { cities: filtered, total: cities.length };
  });

async function buildSiigoCustomerPayload(c: {
  person_type: string;
  id_type: string | null;
  identification: string;
  display_name: string;
  ...
  if (c.address) {
    // Resolver la ciudad real contra el catálogo de Siigo. Siigo espera country_code "Co" (no "COL").
    const resolved = await resolveSiigoCity(c.city_name, c.city_code);
    if (!resolved) {
      throw new Error(
        `No se pudo resolver la ciudad "${c.city_name ?? c.city_code ?? "—"}" contra el catálogo de Siigo. ` +
        "Edita el cliente y elige una ciudad del listado antes de aprobar.",
      );
    }
    payload.address = {
      address: c.address,
...
export const listPendingCustomers = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    if (error) throw new Error(error.message);
    const rows = data ?? [];
    if (rows.length === 0) return { customers: [] as Array<typeof rows[number] & { approval_status: string; requester_name: string | null; requester_email: string | null }> };
    ...
    // Cargar perfil del vendedor solicitante
    const userIds = Array.from(new Set(rows.map((r) => r.created_by_user).filter(Boolean))) as string[];
    ...
    const pending = rows
      ...
      .map((r) => {
        const prof = r.created_by_user ? profileMap.get(r.created_by_user) : null;
        return { ...r, approval_status: statusMap.get(r.id) ?? "pending", requester_name: prof?.full_name ?? null, requester_email: prof?.email ?? null };
      });
...
export const approveCustomer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const merged = {
      ...
      country_code: patch.country_code ?? current.country_code ?? "COL",
      seller_siigo_id: current.seller_siigo_id,
    };

    const payload = await buildSiigoCustomerPayload(merged);
