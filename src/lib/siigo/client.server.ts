...
export const SiigoClient = {
  ...
  /** Llamada genérica autenticada al API de Siigo con retry en 429/5xx. */
  async request<T = unknown>(opts: SiigoRequestOptions): Promise<T> {
    const maxAttempts = 4;
    let lastErr: SiigoApiError | null = null;
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      const text = await res.text();
      if (res.ok) return text ? (JSON.parse(text) as T) : (undefined as T);

      lastErr = new SiigoApiError(res.status, `Siigo ${opts.method ?? "GET"} ${opts.path} falló`, text);
      // Reintentar solo en 429 y 5xx
      const retryable = res.status === 429 || (res.status >= 500 && res.status < 600);
      if (!retryable || attempt === maxAttempts) throw lastErr;

      // Respetar Retry-After si viene, si no backoff exponencial (1s, 3s, 8s)
      let waitMs = attempt === 1 ? 1000 : attempt === 2 ? 3000 : 8000;
      const ra = res.headers.get("Retry-After");
      if (ra) {
        const n = Number(ra);
        if (!Number.isNaN(n) && n > 0) waitMs = Math.min(n * 1000, 30000);
      }
      await new Promise((r) => setTimeout(r, waitMs));
    }
    throw lastErr ?? new Error("Siigo: sin respuesta");
  },
};
