...
export const uploadEvidence = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (error) throw new Error(error.message);
    const { data: signed, error: signErr } = await supabaseAdmin.storage.from("evidence").createSignedUrl(path, 60 * 60 * 24 * 365 * 10);
    if (signErr || !signed?.signedUrl) throw new Error(signErr?.message ?? "No se pudo firmar la evidencia");
    return { url: signed.signedUrl, path };
  });

// Actualiza la geolocalización EXACTA del cliente (sobrescribe el cache).
// Útil cuando el vendedor está físicamente en el local y quiere fijar las
// coordenadas reales del cliente.
export const setCustomerGeo = createServerFn({ method: "POST" })
  .middleware([attachAuthHeader, requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({
    customer_id: z.string().uuid(),
    lat: z.number(),
