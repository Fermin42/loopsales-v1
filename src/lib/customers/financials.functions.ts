...
export const getCustomerFinancials = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      // "Hoy" en Bogotá para comparar sólo por fecha (sin horas ni UTC-drift).
      ...
      invoices = (resp.results ?? []).map((inv) => {
        ...
        // Siigo a veces trae due_date sólo dentro de payments[]; usar fallback.
        const dueRaw = inv.due_date ?? inv.payments?.[0]?.due_date ?? null;
        const dueStr = dueRaw ? String(dueRaw).slice(0, 10) : null;
        let status: "paid" | "pending" | "overdue" = "pending";
        if (balance <= 0.01) status = "paid";
        else if (dueStr && dueStr < todayStr) status = "overdue";
        return {
          id: inv.id,
      ...
      // Orden: vencidas primero, luego sin vencer, pagadas al final. Dentro de cada grupo, por fecha descendente.
      const order: Record<CustomerInvoice["status"], number> = { overdue: 0, pending: 1, paid: 2 };
      invoices.sort((a, b) => {
