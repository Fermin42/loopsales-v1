...
export function ActionDialog({ orderId, action, confirmationMode, onClose, onDone }: Props) {
  ...
  const requestReturn = useServerFn(createOrderRequest);
  ...
  const [deliveryResult, setDeliveryResult] = useState<"accepted" | "rejected">("accepted");
  ...
  const isCustomerDelivery = action.key === "driver_delivers_customer" || action.key === "warehouse_delivers_customer";
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (isCustomerDelivery && deliveryResult === "rejected") {
        await requestReturn({ data: {
          order_id: orderId,
          type: "return",
          reason: observations.trim() || "Cliente rechazó la entrega",
          photo_urls: photoUrls,
          lat: geo.lat, lng: geo.lng, accuracy: geo.accuracy,
        } });
        if (persistKey) clearEvidence(persistKey);
        toast.success("Devolución solicitada para revisión");
        // Cerrar PRIMERO para evitar que la rehidratación de sessionStorage reabra el modal.
        onClose(); onDone();
        return;
      }
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3">
          ...
          {isCustomerDelivery && (
            <div className="space-y-1">
              <label className="text-xs font-medium">Resultado de entrega</label>
              <Select value={deliveryResult} onValueChange={(v) => setDeliveryResult(v as "accepted" | "rejected")}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="accepted">Cliente aceptó</SelectItem>
                  <SelectItem value="rejected">Cliente rechazó</SelectItem>
                </SelectContent>
              </Select>
            </div>
          )}
