...
import { Loader2, Search, Users, MapPin, History, Wallet, Phone, Mail, FileText, Receipt, Package, ExternalLink, Camera, ChevronRight, CheckCircle2, AlertTriangle, Clock, Bell, Crosshair } from "lucide-react";
import { toast } from "sonner";
import { listCustomers } from "@/lib/catalog/catalog.functions";
import { listEventTypes, createCustomerEvent, listCustomerEvents, setCustomerGeo } from "@/lib/customers/events.functions";
...
const VISIT_MODAL_KEY = "vendedor:novedad-modal";
...
function ClientesPage() {
  ...
  const pinGeo = useServerFn(setCustomerGeo);
  ...
  // Rehidratar modal de novedad si el WebView se recicló al volver de la cámara
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      const raw = sessionStorage.getItem(VISIT_MODAL_KEY);
      if (!raw) return;
      const saved = JSON.parse(raw) as { customer: Customer; type: string; notes: string };
      if (saved?.customer) {
        setEvtCustomer(saved.customer);
        setEvtType(saved.type || "");
        setEvtNotes(saved.notes || "");
      }
    } catch { /* ignore */ }
  }, []);

  // Persistir estado del modal mientras esté abierto
  useEffect(() => {
    if (typeof window === "undefined") return;
    if (evtCustomer) {
      sessionStorage.setItem(VISIT_MODAL_KEY, JSON.stringify({ customer: evtCustomer, type: evtType, notes: evtNotes }));
    } else {
      sessionStorage.removeItem(VISIT_MODAL_KEY);
    }
  }, [evtCustomer, evtType, evtNotes]);
  ...
  const submitEvent = async () => {
    if (!evtCustomer || !evtType) return toast.error("Selecciona tipo de novedad");
    setEvtBusy(true);
    try {
      ...
      if (typeof window !== "undefined") sessionStorage.removeItem(VISIT_MODAL_KEY);
  ...
  const confirmPinLocation = async () => {
    ...
    setGeoCustomer(null);
    try {
      const geo = await getGeo(10000);
      if (!geo.lat || !geo.lng) throw new Error("No se obtuvo geolocalización");
      await pinGeo({ data: { customer_id: c.id, lat: geo.lat, lng: geo.lng, accuracy: geo.accuracy ?? null } });
      toast.success(`Ubicación fijada (±${Math.round(geo.accuracy ?? 0)}m)`);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "No se pudo fijar la ubicación");
    }
    setGeoBusyId(null);
  };
  ...
  return (
    <div className="pb-24">
            ...
            {rows.map((c) => (
              <li key={c.id}>
                <Card className="overflow-hidden rounded-2xl border-border/60 active:scale-[0.99] transition-transform">
                  ...
                  <div className="grid grid-cols-4 border-t border-border/60 divide-x divide-border/60 text-[11px] font-medium">
                    ...
                    <button onClick={() => setEvtCustomer(c)} className="py-2.5 flex items-center justify-center gap-1 active:bg-accent/50">
                      <Bell className="w-3.5 h-3.5" />Novedad
                    </button>
                    <button onClick={() => setGeoCustomer(c)} disabled={geoBusyId === c.id} className="py-2.5 flex items-center justify-center gap-1 active:bg-accent/50 disabled:opacity-50">
                      {geoBusyId === c.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Crosshair className="w-3.5 h-3.5" />}Ubicación
                    </button>
      ...
      <Dialog open={!!evtCustomer} onOpenChange={(o) => !o && setEvtCustomer(null)}>
        <DialogContent className="sm:max-w-md rounded-t-3xl sm:rounded-2xl p-0 gap-0 overflow-hidden">
          <DialogHeader className="px-5 pt-5 pb-3 border-b">
            <DialogTitle className="flex items-center gap-2 text-base"><Bell className="w-4 h-4 text-primary" />Registrar novedad</DialogTitle>
      ...
      <Dialog open={!!historyCustomer} onOpenChange={(o) => !o && setHistoryCustomer(null)}>
        <DialogContent className="sm:max-w-md rounded-t-3xl sm:rounded-2xl p-0 gap-0 overflow-hidden">
          <DialogHeader className="px-5 pt-5 pb-3 border-b">
            <DialogTitle className="flex items-center gap-2 text-base"><History className="w-4 h-4 text-primary" />Historial de novedades</DialogTitle>
      ...
      <Dialog open={!!geoCustomer} onOpenChange={(o) => !o && setGeoCustomer(null)}>
        <DialogContent className="sm:max-w-md rounded-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-base">
              <Crosshair className="w-4 h-4 text-primary" />Fijar ubicación exacta
            </DialogTitle>
          </DialogHeader>
          <div className="text-sm space-y-2">
            <p>Se guardarán tus coordenadas actuales como la ubicación oficial de:</p>
            <p className="font-semibold">{geoCustomer?.display_name}</p>
            <p className="text-xs text-muted-foreground">Asegúrate de estar físicamente en el local del cliente.</p>
          </div>
          <DialogFooter className="gap-2">
            <Button variant="outline" className="flex-1 rounded-xl h-11" onClick={() => setGeoCustomer(null)}>Cancelar</Button>
            <Button className="flex-1 rounded-xl h-11" onClick={confirmPinLocation}>
              <Crosshair className="w-4 h-4 mr-1" />Fijar aquí
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
