...
function NuevoPedidoPage() {
  ...
  const [newCustomerCity, setNewCustomerCity] = useState<SelectedCity | null>(null);
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <CityAutocomplete
                      ...
                      inputValue={newCustomer.city_name}
                      onChange={(city, rawText) => {
                        setNewCustomerCity(city);
                        setNewCustomer((s) => ({ ...s, city_name: city?.name ?? rawText }));
                      }}
                    />
                    <p className="text-[10px] text-muted-foreground mt-1">
                      {newCustomerCity
                        ? `✓ ${newCustomerCity.name} (cód. ${newCustomerCity.code})`
                        : "Elige una opción del catálogo Siigo o el sistema usará Bogotá por defecto."}
                    </p>
                  </div>
                </div>
                <div>
              ...
              <DialogFooter className="gap-2">
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    if (!newCustomerCity) return toast.error("Selecciona la ciudad desde el listado (catálogo Siigo) para que se guarde correctamente.");
                    ...
                    try {
                      const r = await createCustomerFn({ data: {
                        ...
                        city_name: newCustomerCity.name,
                        city_code: newCustomerCity.code,
                        state_code: newCustomerCity.state_code,
                        country_code: newCustomerCity.country_code || "COL",
                      }});
