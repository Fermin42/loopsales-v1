...
import { Loader2, FileText, ExternalLink, Pencil, ArrowLeft, AlertTriangle, Ban, CheckCircle2, MapPin, ImageIcon, RotateCcw } from "lucide-react";
import { toast } from "sonner";
import { useAuth } from "@/hooks/use-auth";
import { getOrder, invoiceOrder, cancelOrder, acknowledgeManualPrice, linkExistingSiigoInvoice } from "@/lib/orders/orders.functions";
...
function OrderDetailPage() {
  ...
  // dispatchOrder eliminado: el flujo de reparto se maneja por trazabilidad.
  ...
  // Rehidratar estado de modales tras recarga del WebView (al volver de la cámara nativa).
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      const raw = sessionStorage.getItem(`order-modal:${id}`);
      if (!raw) return;
      const s = JSON.parse(raw) as { sendOpen?: typeof sendOpen; delivOpen?: boolean; reqOpen?: boolean; reqType?: "cancel" | "return"; reqReason?: string; sendUser?: string; sendNotes?: string; delivNotes?: string };
      if (s.sendOpen) setSendOpen(s.sendOpen);
      if (s.delivOpen) setDelivOpen(true);
      if (s.reqOpen) setReqOpen(true);
      if (s.reqType) setReqType(s.reqType);
      if (s.reqReason) setReqReason(s.reqReason);
      if (s.sendUser) setSendUser(s.sendUser);
      if (s.sendNotes) setSendNotes(s.sendNotes);
      if (s.delivNotes) setDelivNotes(s.delivNotes);
    } catch { /* noop */ }
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    const any = sendOpen || delivOpen || reqOpen;
    if (!any) { sessionStorage.removeItem(`order-modal:${id}`); return; }
    sessionStorage.setItem(`order-modal:${id}`, JSON.stringify({
      sendOpen, delivOpen, reqOpen, reqType, reqReason, sendUser, sendNotes, delivNotes,
    }));
  }, [id, sendOpen, delivOpen, reqOpen, reqType, reqReason, sendUser, sendNotes, delivNotes]);
  ...
  const canCancel = (hasRole("admin") && ["draft", "pending", "confirmed"].includes(order.status))
    || (isOwner && ["draft", "pending"].includes(order.status));
  const canAck = hasRole(["admin", "facturacion"]) && order.has_manual_price && !order.manual_price_acknowledged;
  // Solicitud de anulación: solo vendedor dueño, y SOLO después de confirmar y antes de facturar.
  const canRequest = isOwner && hasRole("vendedor") && order.status === "confirmed";
  ...
  const sendNow = async () => {
    ...
    try {
      ...
      const target = sendOpen;
      await doSend({ data: { order_id: id, to_role: target, to_user: sendUser, notes: sendNotes || undefined, photo_url, lat: geo.lat, lng: geo.lng, accuracy: geo.accuracy } });
      clearEvidence(`evidence:handoff:${id}:${target}`);
      // Cerrar el modal ANTES de recargar para evitar rehidratación desde sessionStorage.
      setSendOpen(null); setSendNotes(""); setSendUser(""); setSendPhotoUrls([]);
      toast.success(`Enviado a ${target}`); load();
    } catch (e) { toast.error(e instanceof Error ? e.message : "Error"); }
    setBusy(false);
  };
  ...
  const submitRequest = async () => {
    ...
    // Cerrar el modal PRIMERO para no reabrirlo con la rehidratación de sessionStorage.
    const reason = reqReason.trim();
    const type = reqType;
    setReqOpen(false); setReqReason("");
    await run(() => doRequest({ data: { order_id: id, type, reason } }), "Solicitud enviada");
  };
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 max-w-4xl mx-auto">
        ...
        {canCancel && (
          <Button size="sm" variant="destructive" onClick={() => {
            if (confirm("¿Cancelar este pedido? Se liberará el inventario reservado.")) {
              run(() => doCancel({ data: { id } }), "Pedido cancelado");
            }
          }} disabled={busy}>
            <Ban className="w-4 h-4 mr-1" />Cancelar pedido
          </Button>
        )}
        {canRequest && (
          <Button size="sm" variant="outline" onClick={() => setReqOpen(true)} disabled={busy}>
            <RotateCcw className="w-4 h-4 mr-1" />Solicitar anulación
          </Button>
        )}
      </div>
      ...
      <Dialog open={reqOpen} onOpenChange={setReqOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Solicitar anulación del pedido</DialogTitle></DialogHeader>
          <p className="text-xs text-muted-foreground">Solo disponible antes de facturar. Un administrador revisará y aprobará.</p>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => { setReqType("cancel"); submitRequest(); }} disabled={busy}>Enviar solicitud</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
