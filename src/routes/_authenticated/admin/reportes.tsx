...
import { downloadWorkbookWithCharts } from "@/lib/reports/export-excel";
...
function ReportesPage() {
  ...
  const download = async (key: string, name: string, chartElId?: string, chartTitle?: string) => {
    const d = data[key];
    if (!d) return toast.error("Genera el reporte primero");
    const charts = chartElId ? [{ chartElId, sheet: "Gráfico", title: chartTitle }] : [];
    await downloadWorkbookWithCharts({ Resumen: d.summary, Detalle: d.detail }, `${name}-${from}-a-${to}.xlsx`, charts);
      ...
      <Tabs defaultValue="seller">
        ...
        <TabsContent value="seller" className="space-y-3">
          <div className="flex gap-2">
            ...
            <Button variant="outline" onClick={() => download("seller", "ventas-por-vendedor", "chart-seller", "Ventas por vendedor")} disabled={!data.seller}>
              <Download className="w-4 h-4 mr-1" /> Excel con gráfico
            </Button>
          </div>
          {data.seller && (
            <>
              <Card className="p-3" id="chart-seller">
                <div className="text-sm font-medium mb-2">Total por vendedor</div>
                <div style={{ width: "100%", height: 320 }}>
                  <ResponsiveContainer>
                    <BarChart data={(data.seller.summary as Array<{ seller: string; total: number; orders: number }>).slice(0, 15)}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="seller" tick={{ fontSize: 11 }} angle={-15} textAnchor="end" height={60} />
        ...
        <TabsContent value="product" className="space-y-3">
          <div className="flex gap-2">
            ...
            <Button variant="outline" onClick={() => download("product", "ventas-por-producto", "chart-product", "Top productos")} disabled={!data.product}>
              <Download className="w-4 h-4 mr-1" /> Excel con gráfico
            </Button>
          </div>
          {data.product && (
            <>
              <Card className="p-3" id="chart-product">
                <div className="text-sm font-medium mb-2">Top 15 productos por total</div>
                <div style={{ width: "100%", height: 320 }}>
                  <ResponsiveContainer>
                    <BarChart data={(data.product.summary as Array<{ name: string; total: number }>).slice(0, 15)}>
                      <CartesianGrid strokeDasharray="3 3" />
        ...
        <TabsContent value="customer" className="space-y-3">
          <div className="flex gap-2">
            ...
            <Button variant="outline" onClick={() => download("customer", "ventas-por-cliente", "chart-customer", "Top clientes")} disabled={!data.customer}>
              <Download className="w-4 h-4 mr-1" /> Excel con gráfico
            </Button>
          </div>
          {data.customer && (
            <>
              <Card className="p-3" id="chart-customer">
                <div className="text-sm font-medium mb-2">Top 15 clientes por total</div>
                <div style={{ width: "100%", height: 320 }}>
                  <ResponsiveContainer>
                    <BarChart data={(data.customer.summary as Array<{ customer: string; total: number }>).slice(0, 15)}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="customer" tick={{ fontSize: 10 }} angle={-25} textAnchor="end" height={80} />
                      <YAxis tick={{ fontSize: 11 }} />
                      <Tooltip formatter={(v: number) => formatCurrency(v)} />
                      <Bar dataKey="total" fill="hsl(var(--primary))" name="Total" />
                    </BarChart>
