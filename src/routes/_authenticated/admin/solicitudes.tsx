...
function SolicitudesPage() {
  ...
  const act = async (id: string, decision: "approve" | "reject") => {
    setBusy(id);
    try { await review({ data: { id, decision, notes: notes[id], credit_note_number: creditNotes[id] } }); toast.success("Procesado"); load(); }
        ...
        <Card key={r.id} className="p-4 space-y-2">
          ...
          {r.status === "pending" && (
            <>
              ...
              {r.order?.siigo_invoice_number && (
                <Input placeholder="Consecutivo NC / Nota crédito (obligatorio al aprobar)" value={creditNotes[r.id] ?? ""} onChange={(e) => setCreditNotes((p) => ({ ...p, [r.id]: e.target.value }))} />
              )}
              <div className="flex gap-2">
                <Button size="sm" disabled={busy === r.id || (!!r.order?.siigo_invoice_number && !creditNotes[r.id]?.trim())} onClick={() => act(r.id, "approve")}><Check className="w-4 h-4 mr-1" />Aprobar</Button>
