...
const STATUSES = [
  "all", "pending", "confirmed", "invoiced",
  "in_warehouse", "in_transit", "delivered",
  "returned_to_billing", "with_collections", "finalized",
  "pending_acceptance", "cancelled", "voided",
] as const;
const LABEL: Record<string, string> = { all: "Todos", ...STATUS_LABELS };
...
function AdminPedidosPage() {
  ...
  useEffect(() => {
    setLoading(true);
    // listOrders solo acepta el set viejo; el filtro extendido se hace en cliente.
    fetchList({ data: { scope: "all", limit: 500 } })
